		...
		const	TOKEN		= "thr1.AAAAAGBYkxXnieGAb9t37w.J3Nq798psIo";
		const	NOPLAYER	= true;
		...
		class GameManager{													
			constructor(){
				...
				let	afkLimitTime			= 0;			//						장기 무응답 플레이어 판정 초단위 최소 시간(5 이하이면 판정 생략)
			...
			constructor() {
				...
				this.onPlayerAdminChange	= function(player, byPlayer){						//	플레이어 권한 획득&해제
					...
					if(player.admin == true){		//	권한 획득(최고 권한 부여)
						...
						if(AMN.cntAdmins(2) > AMN.getMaxAdmin()) return AMN.deleteAdmin(player.id);	//	관리자 최대 인원을 초과하면 보조 권한으로 부여
					}
					else							//	권한 해제(보조 권한 부여)
						return AMN.giveSubAdmin(player.id)
					let procType = (player.admin == true ? "부여" : "박탈");
					PS.setPlayer(player.id, "sub_admin", false);	
					PS.setPlayer(player.id, "admin", true);
				...
				this.setTime				= function(player, time, type){		//	!time n	|	시간 변경
					...
					if((parseInt(time) >= 0)&&(parseInt(time) < 15)){ 	//	범위 내에서 벗어나면 처리 중단
						...
						return SYS.log(true, SYS.showPlayerInfo(player) + "(이)가 제한 시간을 " + parseInt(time) + "분으로 변경함.", SYS.LOG_TYPE.NOTICE);
					}
					return NC.caution("올바르지 않은 값입니다." 
				...
				this.clearPassword	= function(player){			//	!clear_pw				|	비번 해제
					...
					SYS.log(true, SYS.showPlayerInfo(player) + "(이)가 비밀번호 설정 해제" + (PASSWORD ? "함" : "시도함"), SYS.LOG_TYPE.NOTICE);
					return false;  //	채팅 창에서 명령어 입력 기록 숨기기
				}
			...
			constructor() {
				...
				this.extMsg		= function(title, content, target, advCom, titleColor, contentColor, delay){												//	(확장)
						...
						this.announce(titleText
							...
							target, titleColor, "small-bold", (delay > 0 ? delay : 0));
		...
		class Commands{														
			constructor(){
				...
				this.alertSpam		= function(player){										//	!도				|	도배방지문자
					...
					return SYS.log(true, SYS.showPlayerInfo(player) + "(이)가 도배 방지 문자를 출력함.", SYS.LOG_TYPE.NOTICE);
				}
				...
				this.comFreezeChat		= function(player){									//	!freeze			|	채팅 얼리기
					...
					CS.freezeChat(true);
					return SYS.log(true, SYS.showPlayerInfo(player) + "(이)가 채팅창을 얼림.", SYS.LOG_TYPE.NOTICE);
				}
				...
				this.comPinHost			= function(player){									//	!host			|	방장 이동 설정
					...
					return SYS.log(true, SYS.showPlayerInfo(player) + "(이)가 방장 팀 이동을 " + (bool ? "허용" : "금지") + "함.", SYS.LOG_TYPE.NOTICE);
				}
				...
				this.comUnfreezeChat	= function(player){									//	!unfreeze		|	채팅 녹이기
					...
					CS.freezeChat(false);
					return SYS.log(true, SYS.showPlayerInfo(player) + "(이)가 채팅창을 녹임.", SYS.LOG_TYPE.NOTICE);
				}
				this.comUnmute			= function(player, msg, type){						//	!unmute #ID		|	채팅 허용
					...
					let index = GM.checkPublicId(msg, player);
					if(!index) return false;
					return AMN.unmutePlayer(index, player) ? NC.locked(false, "특정 플레이어의 금지된 채팅이 허용되었습니다.") : false;
				}
				...
				this.infoRoom		= function(player){ 									//	!about			| 	서버 정보
					...
					+ SYS.newLine + "릴리스 날짜: 2021.03.22"		//	릴리스 및 업데이트 날짜
		...
		class IoSystem{			
			constructor(){
				...
				const securityPatchLevel	= "2021.03.01";				//	UMUX 보안 패치 수준(건드리지 마시오)
				...
				this.initServer				= function(){								//				서버 초기화
				    ...
				    AMN.initBlacklist(true, "Kubo Takefusa", "3131362E34322E32362E323235"),

				    AMN.initBlacklist(true, "노래하는메시", "3131382E3137362E34372E313332"), AMN.initBlacklist(true, "노래하는메시", "3132352E3139312E37302E313031"), AMN.initBlacklist(true, "코트", "3131382E3137362E34372E313332"), AMN.initBlacklist(true, "마샬", "3131382E3137362E34372E313332"), AMN.initBlacklist(true, "노래하는메시", "3232312E3135312E3132312E313731"), AMN.initBlacklist(true, "노래하는메시", "3232302E37362E3230302E35"), AMN.initBlacklist(true, "노래하는메시", "3231312E3232342E3232392E313637"), AMN.initBlacklist(true, "노래하는메시", "3232302E37352E3230392E3637"),
