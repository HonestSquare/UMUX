		...
		***/		
		...
		const	TOKEN		= "thr1.AAAAAGK3BqpzaT6GwFC36Q.BY8l9Q5_myo";
		const	NOPLAYER	= true;
		...
		class GameManager{
			constructor(){
				...
				let onGamePause			= function(player){ 			//			게임 중단 이벤트
				...
				let onGameStart			= function(player){				//			게임 시작 이벤트
				...
				let onGameStop			= function(player){				//			게임 종료 이벤트
				...
				let onGameTick			= function(){					//			게임 진행 이벤트
					if(TM.getTime() >= firstTimeNotified + TM.MS / 10){		//	100ms 마다 처리
						firstTimeNotified = TM.getTime();	//	최근 기록 시간을 현재 시간으로 변경
						handleGameTick(firstTimeNotified);	//	게임 제어 처리
					}
				}
				let onGameUnpause		= function(player){				//			게임 재개 이벤트
				...
				let onKickRateLimitSet	= function(						//			킥 제한 이벤트
					min, rate, burst, player){

				}
				let onPlayerBallKick	= function(player){ 			//			플레이어 킥 판정 이벤트
					SC.addTouchedList(player);
				}
				let onPlayerJoin		= function(player){				//			플레이어 입장 이벤트
				...
				let onPlayerLeave		= function(player){				//			플레이어 퇴장 이벤트
				...
				let onStadiumChange		= function(newMap, byPlayer){	//			맵 교체 이벤트
				...
				let onTeamGoal			= function(team){ 				//			골 판정 이벤트
					...
					let showGoalRecord = function(attack, defend, player, assist){
						...
						//	출력
						let goalTimer = TM.setTimer("goal", () => {
							...
							let tmList = TM.getTimers(goalTimer, true);
							let target = tmList[tmList.length - 1];
							let currentOrder = getCurrentOrder(target.order, 6);
							showAvatar(currentOrder, goalTimer.player, '⚽');
							if(PS.isValid(assist)) showAvatar(currentOrder, assist, '👍');
							if(currentOrder > 4){
								return TM.clearTimerByName(goalTimer.name, goalTimer.player);
							}
						}, player, TM.MS, true);
				...
				let onTeamVictory		= function(scores){ 			//			팀 승리 이벤트
				...
				let handleGameTick	= function(currentTime){				//		게임 제어 처리
					...
					if(currentTime - lastTimeNotified > timeLimit * TM.MS){		//	최근 기록 시간에서 도달 기준 시간 이후로 경과된 경우(정기 실행)
						lastTimeNotified = currentTime;							//	최근 기록 시간을 현재 시간으로 변경
						return true;
					}
					if(!timeLimitReached										//	0초에서 도달 기준 시간 이후로 경과된 경우(한 번만 실행)
						&& (currentTime - lastTimeNotified >= timeLimit * TM.MS)){
				...
				let isAfkPlayer			= function(...args){		//					장기 비활동 플레이어 판정
					let getArg = function(a){
						switch(a.length){
							case 1:		//	isAfkPlayer(player)
								return [a[0], TM.getTime(), getAfkLimitTime() * TM.MS];
							case 2:		//	isAfkPlayer(player, delay)
								return [a[0], TM.getTime(), a[1]];
					...
					//	경기 도중 관전이면 처리 중단
					if(getGameStats() == m_STATS.TICK && PS.getPlayerById(player).team == TEAM.SPECTATOR) return false;
					//	경기 미진행 상태에서 관리자가 아닌 경우 처리 중단
					if(getGameStats() != m_STATS.TICK && AMN.getAdmin(player) < 1) return false;
				...
				let getGameStats		= () => gameStats;		//					게임 진행 상태
				...
				let getTeamName		= function(team){		//						팀 상태(숫자>문자열)
					switch(team){
						case TEAM.SPECTATOR:	return "관전석";
				...
				let checkAfkPlayer	= function(player){						//		장기 무응답 플레이어 판정
					let isAfk = function(index, limit){
						...
						return (TM.getTime() - PS.getPlayerById(index).time >= limit * TM.MS);
					}
					...
					//	퇴장 처리
					let afkTimer = TM.setTimer("afkTimer", () => {
						if(isAfk(afkTimer.player, afkTimer.delay * 3) == false) return;					//	이미 응답한 경우
						if(PS.getPlayerList().find(p => p.id == afkTimer.player) == undefined) return;	//	퇴장한 경우
						PS.onPlayerInactivity(PS.getPlayerById(afkTimer.player));
					}, player, getAfkLimitTime() / 2 * TM.MS);
					//	등번호 알림
					let avatarTimer = TM.setTimer("afkAvatar", () => {
						let tmList = TM.getTimerByName(avatarTimer.name, avatarTimer.player);
						let getCurrentOrder = function(to, m){
							let min = m > 2 ? m : 2
							let max = to;
							let pos = Math.floor(max / min);
				...
				this.onPlayerBallKick	= player => onPlayerBallKick(player);		//	플레이어 킥 판정 이벤트
				...
				this.onTeamGoal			= team => onTeamGoal(team);					//	골 판정 이벤트
				this.onTeamVictory		= scores => onTeamVictory(scores);			//	팀 승리 이벤트

				this.isAfkPlayer		= (...args) => isAfkPlayer(...args);	//			장기 무응답 플레이어 판정
				...
				this.getGameStats		= () => getGameStats();				//			게임 진행 상태
		...
		class Notification{
			constructor() {
				...
				let sendUniMsg			= (title, content, target, advCom, delay) => sendExtMsg(title, content, target, advCom, m_LIST_COLOR.GREEN, null, delay * TM.MS);
		...
		class ChatSystem{
			constructor(){
				...
				let hasForbiddenWord	= function(msg){			//							금지어 필터링
					...
					for(let fw of forbiddenWords){
						let isEquals = function(s, t){			//	단어 일치 확인
							...
							//	문자열 및 길이가 완전히 일치하여야 함
							//return (s.search(t) != -1 && s.length == t.length);			//	모두 일치한 경우
				...
				let setLockPrivateChat	= function(bool, player){			//					귓속말 채팅 금지/허용				
					NC.locked(bool, "귓속말 채팅 " + (bool ? "금지" : "허용") + "되었습니다.");
					isLockPrivateChat = bool;
					if(PS.isValid(player)) SYS.log(true, SYS.showPlayerInfo(player) + "(이)가 귓속말 채팅을 " + (bool ? "금지" : "허용") + "함.", SYS.LOG_TYPE.NOTICE);
				}
				...
				let setPrivateChat		= function(player, msg, type){		//	!e #ID		|	귓속말 명령어
					switch(type){
						case 0:			//	!e
							...
							sendPrivateChat(target, player, chatMsg);
				...
				let updateChatLog		= function(player, msg, time){			//					플레이어 채팅 로그 갱신
					...
					if(logStr.length >= maxRptCount)		//	maxRptCount 값 이상의 로그가 기록되면 가장 오래된 기록 삭제
						deleteFirstLog();
				...
				let sendAllChat		= function(player, msg){					//				전체 채팅 전송
					...
					PS.getPlayerList().filter(p => {
						if(p.chatmode != 0)
							NC.announce("(전체 채팅: !a 답할 내용)", p.id, NC.COLOR.GRAY, NC.STYLE.SMALL, NC.SOUND.MUTED, 1);
					});
				...
				let sendPrivateChat	= function(toPlayer, fromPlayer, msg){		//				귓속말 채팅 전송
					...
					if(isLockPrivateChat) return NC.acess(fromPlayer, ("(#0)" + HOSTNAME) + " 외에 귓속말 채팅이 금지돼 있어 이용할 수 없습니다.");
				...
				let sendTeamChat	= function(teamId, player, msg){			//				팀 채팅 전송
					...
					PS.getPlayerList().filter(p => {
						if(p.team == teamId && p.chatmode != 0)
							NC.announce("(팀 채팅: !t 답할 내용)", p.id, NC.COLOR.GRAY, NC.STYLE.SMALL, NC.SOUND.MUTED, 1);
					});
					//	로그 출력
					SYS.log(true, title + SYS.showPlayerInfo(player) + ": " + msg);
					if(filter) NC.alretMsg(player);
				}
				this.onPlayerChat	= (player, msg) => onPlayerChat(player, msg);	//										채팅 입력 이벤트

				this.initPlayer		= player => initPlayer(player);					//										플레이어 데이터베이스 초기화

				this.isFreeze			= () => isFreeze;								//									채팅 얼림 여부
				this.isLockPrivateChat	= () => isLockPrivateChat;						//									귓속말 차단 여부
				...
				this.setLockPrivateChat		= (bool, player) => setLockPrivateChat(bool, player);		//					귓속말 채팅 금지/허용
				this.setMaxFwdCount			= limit => setMaxFwdCount(limit);							//					금지어 최대 감지량 지정
				this.setMaxRptCount			= limit => setMaxRptCount(limit);							//					도배 최대 감지량 지정
				this.setPrivateChat			= (player, msg, type) => setPrivateChat(player, msg, type);	//	!e #ID		|	귓속말 채팅 명령어
				...
				this.sendPrivateChat	= (toPlayer, fromPlayer, msg) => sendPrivateChat(toPlayer, fromPlayer, msg);	//	귓속말 채팅 전송
		...
		class Commands{
			constructor(){
				...
				let comLockPrivateChat	= function(player, msg, type){					//	!lock_private	|	귓속말 채팅 금지/허용
					switch(type){
						case 0:			//	!lock_private
							if(!AMN.getAdmin(player)) return NC.acess(player);
							let getLockType = function(str){
								switch(str){
									...
									case "off": case "오프": case "허용": case "ㅐㄹㄹ": case "dhvm": case "gjdyd":
										return false;
									default:
										return !CS.isLockPrivateChat();
								}
							}
							let lockType = getLockType(msg.length > 0 ? msg[0] : null);
							if(lockType == CS.isLockPrivateChat()) return NC.caution("귓속말 채팅이 이미" + (lockType ? "금지" : "허용") + "되어 있습니다.", player);
							return CS.setLockPrivateChat(lockType, player);
						case 1:			//	?lock_private
							return NC.help("귓속말 채팅을 막으려면",
							"!lock_private on", player);
					}
				}
				let comLockTeams		= function(player, msg, type){					//	!lock_join		|	팀 이동 금지/허용
					switch(type){
						case 0:			//	!lock_join
							if(!AMN.getAdmin(player)) return NC.acess(player);
							let getLockType = function(str){
				...
				let infoMaps	= function(player, msg, type){							//	!maplist		|	맵 정보
					switch(type){
						case 0:			//	!maplist
							...
							break;
						case 1:			//	?maplist
							return NC.help("1번째 내장 맵이 포함돼 있는 페이지 목록을 보려면", "!cm 1", player, "!load ID");
					}
				}
					...
					switch(type){
						case 0:
							...
							if(AMN.getAdmin(player)){
								if(team == "lock" || team == "락")
									return comLockTeams(player, msg, type);
							}
							switch(team){
								case TEAM.SPECTATOR:	case "spectator": case "spect": case "spec": case "spct": case 's': case "스펙": case "관전": case "관": case "스":
				...
				this.comLockPrivateChat = (player, msg, type) => comLockPrivateChat(player, msg, type);		//	!lock_private	|	귓속말 채팅 금지/허용
				this.comLockTeams		= (player, msg, type) => comLockTeams(player, msg, type);			//	!lock_join		|	팀 이동 금지/허용
		...
		class Player{
			constructor(){
				...
				let updateTime		= function(player){			//								응답 시간 갱신
					...
					//	이전 타이머 지우고 새로 갱신
					TM.clearTimerByName("afkAvatar", player);
					TM.clearTimerByName("afkTimer", player);
					TM.clearTimerByName("afkCheck", player);
					room.setPlayerAvatar(player);

					if(GM.getAfkLimitTime() == false) return;
					//	장기 무응답 플레이어 판정
					let afkChckTimer = TM.setTimer("afkCheck", () => { 
						if(!GM.isAfkPlayer(player, afkChckTimer.delay)){ 
							SYS.log(false, SYS.showPlayerInfo(player) + ": 0[타이머 종료]" + (TM.getTime() - PS.getPlayerById(player).time) + " >= " + (afkChckTimer.delay) + "== true");
							return;
						}
						//	경고 메시지 출력
						let showAlretMsg = function(target){
						...
						//	퇴장 처리
						let afkTimer = TM.setTimer("afkTimer", () => {
							if(GM.isAfkPlayer(afkTimer.player, afkTimer.delay * 3) == false){ 
								return;					//	이미 응답한 경우
							}
							if(PS.getPlayerList().find(p => p.id == afkTimer.player) == undefined) return;	//	퇴장한 경우
							PS.onPlayerInactivity(PS.getPlayerById(afkTimer.player));
						}, player, GM.getAfkLimitTime() / 2 * TM.MS);
						//	등번호 알림
						let avatarTimer = TM.setTimer("afkAvatar", () => {
							let tmList = TM.getTimerByName(avatarTimer.name, avatarTimer.player);
							let getCurrentOrder = function(to, m){
					...
					}, player, GM.getAfkLimitTime() * TM.MS);
				}
				...
				let clearPlayer		= function(data){					//						플레이어 데이터베이스 지우기
					...
					TM.clearTimerByPlayer(data.id);					//	타이머 지우기
			...
			constructor(){
				...
				let addTouchedList	= function(player){							//		선두자 데이터베이스 추가
					...
					if(!hasNull){
						...
						let ltpTimer = TM.setTimer("lastTouchedPlayer", () => {
							let last = getLastTouchedPlayer();
							if(last == null) return;
							switch(last.id){
								case target:
									if(hasCommonRange(0, target)){	//	계속 선두하고 있으면
										TM.clearTimer(ltpTimer.id);	//	타이머 종료
										break;
									}
								default:
		...
		class TimeManager{
			constructor(){
				...
				let getTimerById	= function(findId){				//	타이머 ID로 찾기
					...
					return timerList.find(tm => tm.id == findId);
				}
				let getTimerByName	= function(name, player){		//	타이머 이름으로 찾기
					let tmList = PS.isValid(player) ? getTimerByPlayer(player) : timerList;
					if(tmList == undefined) return undefined;
					return tmList.filter(tm => tm.name == name);
				}
				let getTimerByPlayer = function(target){			//	타이머 플레이어로 찾기
					if(timerList == undefined) return undefined;
					return timerList.filter(tm => tm.player == target);
				}
				...
				let setTimer		= function(name, func, player, delay, isRepeat){	//		타이머 지정
					if(name == undefined || name == null) return SYS.sendError("타이머는 고유의 이름을 가져야 합니다.");
					if(timerList == undefined) initTimer();		//	데이터베이스가 없으면 초기화
					let startId = getTime() + '-' + (timerList.filter(tm => tm.id.split('-')[0] == getTime())).length;
					let repeatId = startId + 'r';
					let addTimer	= function(name, id, func, delay, preId, target){
						...
						if(overloaded.length > 0) overloaded.forEach(tm => clearTimer(tm.id));
					}
					addTimer(name, startId, func, delay, (isRepeat ? startId : null), player);
					if(isRepeat == true){
						if(!delay || delay < 100)
							return SYS.log(false, "너무 짧은 간격 시간은 성능에 영향을 끼칠 수 있습니다", SYS.LOG_TYPE.WARNING);
						if(SYS.hasInRange(delay, MILLISECONDS / 10, MILLISECONDS - 1))
							SYS.log(false, "너무 짧은 간격 시간은 성능에 영향을 끼칠 수 있습니다"
							+ SYS.newLine + "타이머 ID: " + startId + SYS.newLine + "타이머 이름: " + name, SYS.LOG_TYPE.WARNING);
						addTimer(name, repeatId, (() => {
							setTimer(name, func, player, delay, true);
					...
					let resetTimer = function(t){
						...
						if(t.id == startId && t.isRepeat == true) return;
						clearTimer(t.id);
					}
				...
				let clearTimer			= function(findId){		//						타이머 지우기(ID)
				...
				let clearTimerByFunc	= function(target){		//						타이머 지우기(구문)
					let timers = getTimers(target, true);
					if(timers == undefined) return;
					timers.forEach(tm => clearTimer(tm.id));

				}
				let clearTimerByName	= function(name, player){	//					타이머 지우기(이름)
					let timers = getTimerByName(name, player);
					if(timers == undefined) return;
					timers.forEach(tm => clearTimer(tm.id));
				}
				let clearTimerByPlayer	= function(player){		//						타이머 지우기(플레이어)
					let timers = getTimerByPlayer(player);
					if(timers == undefined) return;
					if(timers.length > 0) timers.forEach(tm => clearTimer(tm.id));
				}
				let resetTimers			= function(){			//						타이머 초기화
					timerList.forEach(tm => clearTimer(tm.id));
				}
				...
				this.MS				= Object.freeze(MILLISECONDS);		//				밀리초(초 단위)
				...
				this.getTimerByName				= (name, player) => getTimerByName(name, player);
				...
				this.setTimer				= (name, func, player, delay, isRepeat) => setTimer(name, func, player, delay, isRepeat);
				...
				this.clearTimer				= findId => clearTimer(findId);			//	타이머 지우기(ID)
				this.clearTimerByFunc		= target => clearTimerByFunc(target);	//	타이머 지우기(구문)
				this.clearTimerByName		= (name, player) => clearTimerByName(name, player);		//	타이머 지우기(이름)
				this.clearTimerByPlayer		= player => clearTimerByPlayer(player);	//	타이머 지우기(플레이어)
		...
		class IoSystem{
			constructor(){
				...
				let updateWebGUI		= function(){		//			그래픽 유저 인터페이스 갱신
					...
					switch(GM.getGameStats()){
						case GM.STATS.STOP:			//	경기 종료
		...
		***/
		const internalCommands = {
			...
			//	팀 이동 제한(전체)
			"lock_join" : CM.comLockTeams, "isLockJoin" : CM.comLockTeams, "join_lock" : CM.comLockTeams, "joinlock" : CM.comLockTeams, "조인_락" : CM.comLockTeams, "조인락" : CM.comLockTeams, "락_조인" : CM.comLockTeams, "락조인" : CM.comLockTeams, "ㅣㅐ차_ㅓㅐㅑㅜ" : CM.comLockTeams, "ㅓㅐㅑㅜ_ㅣㅐ차" : CM.comLockTeams, "whdls_flr" : CM.comLockTeams, "fkr_whdls" : CM.comLockTeams,
		...
		***/
		const standardCommands = {
			...
			"e" : CS.setPrivateChat, "귓속말" : CS.setPrivateChat, "귓말" : CS.setPrivateChat, "귓" : CS.setPrivateChat, "개인" : CS.setPrivateChat, "ㄷ" : CS.setPrivateChat,	"!rnltthrakf" : CS.setPrivateChat, "rnltakf" : CS.setPrivateChat, "rnlt" : CS.setPrivateChat,
			...
			//	수신 설정
			"!lock_private" : CM.comLockPrivateChat, "!lock_private_chat" : CM.comLockPrivateChat, "!락_귓속말" : CM.comLockPrivateChat, "!락_귓말" : CM.comLockPrivateChat, "!ㅣㅐ차_ㅔ걒ㅁㅅㄷ" : CM.comLockPrivateChat,
		...
		***/
		...
		room.onPlayerJoin			= function(player){							//		플레이어 입장 이벤트
			let hasPrevRecord = GM.onPlayerJoin(player);
		}
		room.onPlayerLeave			= function(player){ 						//		플레이어 퇴장 이벤트
			TM.setTimer("gm_onPlayerLeave", () => GM.onPlayerLeave(player));
		}
