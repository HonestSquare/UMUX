function log(io, msg){				// 방에서 콘솔창으로 출력
	if(msg){
...
//-----------------------------------------------여기서부터 복사-----------------------------------------------------
var date = new Date();
var VersionRoom = 'v1.00';			// 봇방 버전
var VersionUX  = '1.1.2';			// UMUX 버전(건드리지 마시오)
...
class GameManager{													// 게임 매니저 클래스
	constructor(){
		...
		this.onGameTick = function(){							// 게임 도중
			if(game.gamestats != 2) game.gamestats = 2;
			return false;
		}
		...
		this.onGamePause = function(byPlayer){ 					// 게임 중단
			if(byPlayer.id) log(true, byPlayer.name + '(이)가 게임을 일시 중단함.');
			else log(true, '게임 일시 중단.');		// 명령어 입력으로 실행되면 플레이어 이름 가림
			return false;
		}	
		this.onGameUnpause 	= function(byPlayer){				// 게임 재개
			if(byPlayer.id) log(true, byPlayer.name + '(이)가 게임을 재개함.');
			else log(true, '게임 재개.');			// 명령어 입력으로 실행되면 플레이어 이름 가림
			return false;
		}
		...
		this.onPlayerLeave = function(player) {											// 플레이어 퇴장
			...
			else room.stopGame();								// 사람이 없으면 자동 종료
			return false;
		}
...
class Administration{												// 관리 클래스
	constructor() {
		...
		this.onPlayerKicked = function(kickedPlayer, reason, ban, byPlayer){			//	플레이어 강제 퇴장
			admin.kicked = true;
			if(reason) log(true, byPlayer.name + '(이)가 ' + kickedPlayer.name + '(을)을 강제 퇴장함. (사유: ' + reason + ')');
			else log(true, byPlayer.name + '(이)가 ' + kickedPlayer.name + '(을)을 강제 퇴장함.');
			return admin.kicked;
		}
			...
			if ( players.length == 0 ) return; 
			  ...
			  if(admin.dynamicAdmin){
					if(admin.filterPlayer(player)) return admin.getSubAdmin(player);
						  ...
						  log(true, player.name + '(이)가 권한을 얻음.');
				  }
			  }
		}
				...
				if(!game.gamestats){
					if((msg.substr(6)) >= 0){
						if((msg.substr(6)) < 15){
							...
							log(true, player.name + '(이)가 제한 점수를 ' + msg.substr(6) + '점으로 변경함.');
						}
					}
				}
				...
				if(!game.gamestats){
					if((msg.substr(6)) >= 0){
						if((msg.substr(6)) < 15){
							...
							log(true, player.name + '(이)가 제한 시간을 ' + msg.substr(6) + '분으로 변경함.');
						}
					}
				}
				else n.announce(n.cnot() + '판이 완전히 끝난 이후에 다시 시도해 보세요.', player.id, 'orange', 5, 3);
		...
		this.setPassword = function(player, pass){			// !set_pw string			|	비번 설정
			if(player.admin || admin.subAdmin[player.id - 1]){
				if(pass.length <= 10) n.announce(n.locked() + '비밀번호가 너무 짧습니다. ' + com.recommendCom('연관', '!clear_pw'), player.id, 'orange', 5, 0);
					...
					log(true, player.name + '(이)가 비밀번호를 설정함. (' + admin.password + ')');
				}
			}
			else n.acess(player);
			return false;
		...
		this.missPassword = function(player){ 				// 								어드민 전용 명령어 오타 방지
			log(true, player.name + '(이)가 관리자 로그인을 시도함. (실패)');
			return false; 
		}
		...
		this.getSubAdmin = function(player){				// 								보조 권한 획득
			if (!player.admin){
				...
				log(true, player.name + '(이)가 보조 권한을 얻음.');
			}
			return false; // 명령어 입력한 흔적이 남지 않음
		}
		...
		this.clearBans = function(player){					// !clearbans				|	강제 퇴장 목록 초기화 명령어
			if (player.admin || admin.subAdmin[player.id - 1]){
				...
				log(true, player.name + '(이)가 영구 퇴장 목록을 모두 지움.');
			}
			else n.acess(player);
			return false;  // 명령어 흔적을 남기지 않음
		}
		...
		this.releasePassword = function(player){			// !clear_pw				|	비번 해제
			if(player.admin || admin.subAdmin[player.id - 1]){
				if(!admin.password){
					...
					log(true, player.name + '(이)가 비밀번호 해제를 시도함.');
				}
				else{
					admin.updatePassword();
					n.announce(n.unlocked() + '비밀번호가 해제되었습니다.', null, 'pink', 4, 1);
					log(true, player.name + '(이)가 비밀번호를 해제함.');
				}
			}
			else n.acess(player);
			return false;
		...
		this.filterPlayer = function(player){				//								사칭 및 중복 필터(onlyadmin)
			...
			if(p.cntPlayers > 1){											// 사칭 감지
				for (let j = 1; j < p.cntPlayers; j++){
					//if((p.getIdofPlayer(p.members[j]) != p.playersid[player.id])&&(p.members[j].name == player.name))
						...
						if((msg.substr(6) > 0) && (msg.substr(6) <= p.cntPlayers)){
							if(p.members[msg.substr(6)] && msg.substr(6) <= p.cntPlayers){    
								...
								if(p.members[msg.substr(6)].id == player.id) return n.announce(n.cnot() + '자기 자신의 채팅을 금지할 수 없습니다.', player.id, 'orange', 5, 3)
									...
									log(true, msg.substr(6) + '(이)의 채팅이 금지됨.');
								}
							}
							else return n.announce(n.cnot() + '잘못된 대상입니다.', player.id, 'orange', 2, 3);
						}
		...
		this.releaseMute = function(player, msg)		// !unmute ID			|	채팅 허용(onlyadmin)
		{
			if (player.admin || admin.subAdmin[player.id - 1]){
				...
				log(true, player.name + '(이)가 모든 채금을 해제함.');
				return false;
			}
			else return n.acess(player);
		}
...
class ChatSystem {													// 채팅 시스템 클래스
	constructor(){
		...
		this.ffWords = [												// 							욕설 필터링 단어
			...
			"애미 뒤졌네","애1미 뒤졌네","애2미 뒤졌네","애미 뒤졌다","애1미 뒤졌다","애2미 뒤졌다",
		]
		...
		this.filterWords = function(player, msg){						//							욕설 필터링
			...
			while(i < chat.fWords.length){			// 경고 및 주의
				...
				if(msg.search(chat.fWords[i]) !== -1){ 
					...
					log(true, " 전체 : [" + p.playersid[player.id] + "]🚫" + player.name + ": " + msg);
		...
		this.send = function(player, msg, chatPublic){					//							일반 채팅
			var fmsg = (" 전체 : [" + p.playersid[player.id] + "]");
			...
			for(var i = 0; i <= p.cntPlayers; i++){
				if(p.members[i] && (i == msg.substr(3, 2))){
					...
					n.announce(n.cnot() + '자기 자신에게 귓속말을 보낼 수 없습니다.', player.id, 'orange', 2, 3);
						...
						log(false, "[" + p.playersid[player.id] + "]" + player.name + ": " +  msg.substr(5, 50));
					}
					else{
						...
						log(true, "개인 : [" + p.playersid[player.id] + "]" + p.checkMarks(player) + player.name + "→ [" + p.playersid[p.members[i].id] + "]" + p.checkMarks(p.members[i]) + p.members[i].name + ": " + msg.substr(5, 50));
					}
				}
			}
			// 귓속말 처리
		...
		this.teamChat = function(player, msg)							// !t 					|	팀 채팅 명령어
		{
			...
			room.sendChat(toTeam + " : [" + p.playersid[player.id] + "]" + p.checkMarks(player) + player.name + ": " + msg.substr(3, 50), player.id);
			log(true, toTeam + " : [" + p.playersid[player.id] + "]" + p.checkMarks(player) + player.name + ": " + msg.substr(3, 50));
			
			for(let i = 1; i <= p.cntPlayers; i++){
				// 플레이어를 제외한 나머지 팀원 찾기
				if(p.playersid[p.members[i].id] != p.playersid[player.id]){
					if(p.getTeamofPlayer(p.members[i]) == player.team)
					room.sendChat(toTeam + " : [" + p.playersid[player.id] + "]" + p.checkMarks(player) + player.name + ": " +  msg.substr(3, 50) + " (팀 채팅: !t 답할 내용)", p.members[i].id);
				}
			}
			return false;
		}
	...
	constructor(){
		this.plaster = function(player){										// !도			|	도움말, 도배방지문자(onlyadmin)
    		if (player.admin || admin.subAdmin[player.id - 1]){ 
				...
				room.sendChat(chat.getFace(chat.face[9], 'AlphaGo') + ': ');
				log(true, player.name + '(이)가 도배 방지 문자를 출력함.');
			}
		...
		this.botHelp = function(player, msg){									// !bothelp		|	봇방 도움말
			...
			for(var i = 1; i <= p.cntPlayers; i++){
				if(players[i].admin || admin.subAdmin[player.id - 1]){
					...
					+ '!about(봇방 정보) | !set_pw(비번 설정) | !clear_pw(비번 해제) | !show_pw(비번 표시) | !도(도배 방지 문자)', players[i].id, null, 4, 3);
				}
				else{
					n.announce(n.msgCommand('봇방 ') 
		...
		this.etcHelp = function(player, msg){									// !etchelp		|	기타 도움말
			...
			for(var i = 1; i <= p.cntPlayers; i++){
				if(p.members[i].admin || admin.subAdmin[player.id - 1]){
					room.sendChat(n.mesCommand('기타 ') 
					+ '!avatar(등번호 변경) | !adminhelp(관리 도움말)', p.members[i].id);
				}
				else{
					room.sendChat(n.mesCommand('기타 ') 
					+ '!avatar(등번호 변경)', p.members[i].id);
				}
			}
			return false;
		}
		...
		this.infoRoom = function(player, msg){ 									// !info		| 	방 정보
			chat.send(player, msg, true);
			n.announce(n.info() + '봇방입니다. | 버전: ' + VersionRoom + ' (Based by UMUX ' + VersionUX + ') | 최종 업데이트: 19/08/17 | ' + com.recommendCom('관련', '!help'), null, 'green', 5, 3);
			return false;
		}
		...
		this.loadMap = function(player, mapId){									// !load n		|	유즈맵 로드 명령어(onlyadmin)
			if (player.admin || admin.subAdmin[player.id - 1]){
				if((mapId.substr(6, 2)) >= 1 && ((mapId.substr(6, 2)) <= 12)){
					...
					log(true, player.name + '(이)가 ' + mapId.substr(6, 2) + '번 맵으로 교체함.');
				}
				else{ 
					n.announce(n.cnot() + "1~12 안의 범위를 입력하세요.", player.id, 'orange', 5, 1);
					log(true, player.name + '(이)가 맵 교체를 시도함 (실패).');
				}
			}
			else n.acess(player);
			return false;
...
class Player{														// 플레이어 클래스
	constructor() {
		...
		this.getIdofPlayer = function(player) {					// 플레이어 ID 확인
			...
			ids = p.getNumbers();
			for(var i = 1; i <= 15; i++) {
				if((player.id) && (p.members[i].id == player.id))
					return ids[i - 1];
			}
			ids = null;
		};
		...
		this.getMarks = function(pub, player, fmsg, lmsg){		// 플레이어 마크 부여
			log(true, (fmsg + p.checkMarks(player) + lmsg));
...
var commands = {					// 명령어 모음
	...
	"!maphelp" : com.mapHelp, "!맵도움" : com.mapHelp, "!맵도움말" : com.mapHelp, "!맵헬프" : com.mapHelp, "!ㅡ메ㅗ디ㅔ" : com.mapHelp, 
	...
	// 이스터 에그
	"!joke": com.joke, "!농담": com.joke, "!조크": com.joke, "!넝담": com.joke, "!알파고": com.infoAg, "!네모형": com.jongikannemohyung, "!정네": com.jongikannemohyung,
