...
`{
	...
	//	SUPPORT_LEVEL:	
	//					1(1.0.0)/2(1.1.0)/3(1.1.1)/4(1.1.2)/5(2.0.0)
...
constructor(){
	...
	this.sendWhisperChat = function(toPlayer, fromPlayer, msg){		//							귓속말 채팅
		...
		if((toPlayer == 0) &&(fromPlayer != 0)){					// ID가 0이면 콘솔창으로 전달
			...
			SYS.log(true, "개인 " + spid + '(' + slid + ')' + mark + PS.members[fromPlayer].name + '→ ' + dpid + '(' + dlid + ')' + PS.checkMarks(PS.members[toPlayer]) + PS.members[PS.getPublicId(toPlayer)].name + ": " + msg);
		}
		return false;
	}
...
constructor(){
	...
	this.scoreHelp = function(player, msg){ 								// !rankhelp	| 	랭크 도움말
		...
		for(let i = 1; i <= PS.cntPlayers; i++){
			if(AMN.getAdminstats(PS.members[PS.getPublicId(i)])){
				...
				+ "!playlist(플레이어 목록)", PS.getPublicId(i), null, 4, 3);
			}
			else{
				NC.announce(NC.msgCommand("랭크 ") 
				+ "!playlist(플레이어 목록)", PS.getPublicId(i), null, 4, 3);
	...
	this.infoRoom = function(player, msg){ 									// !info		| 	방 정보
		...
		+ ' | ' + "릴리스 날짜: 2019.12.28" 	// 릴리스 및 업데이트 날짜
		, null, "green", 5, 3);
		// 지우지 마시오
...
constructor() {
	...
	this.superBlacklist = [							// 슈퍼 블랙리스트 명단
		...
		// 농협신, 쁘이훈
		"3132342E35332E3137362E3831", "3132352E3137392E3231312E3330", "3132352E3137392E3231312E3331", "3131382E3137362E34372E313233", "3132352E3137392E3231312E3232", "3132352E3137392E3231312E3533",

		// 노래하는메시, 페페, 마샬
		"3131382E3137362E34372E313332", "3132352E3139312E37302E313031", "3232312E3135312E3132312E313731", "3232302E37362E3230302E35", "3231312E3232342E3232392E313637", "3232302E37352E3230392E3637", "3136332E3138302E3131382E313734", "3231312E3230342E3132352E323430", "35382E3233332E38302E3532", "3138332E3130322E34332E313735", "3132312E3139302E3233332E313635", "3131392E3139322E3235342E323438", "3132312E3134332E3133342E3637", "3232322E3131322E34392E313630",
		"312E3233312E36322E313335", "3232302E37322E39362E3637",
		...
		// 드록바 drogba
		"3131382E33322E37372E323531", "35382E3134332E37362E3635",
		...
		// HYNN
		"3231392E3130302E33372E323433", "3232322E3130352E302E313733", "3231382E35312E31392E3338",
	...
	this.blacklist = [ 								// 블랙리스트 명단
		"에드", "핑폭테러단", "Walker", "페르난지뉴", "앙헬리노", "Knife", "웨인루니", "가즈으앗", "플레이보이카티", "Aaron Wan-Bissaka",
		"강퇴하면핑폭", "랄랄랄", "james", "어드안주면핑터짐", "어드 안주면 핑폭함 ㅅㄱ", "핑폭테러단 인원 모집", "제몸무게가 220kg인데 정상인가요", 
		"와이어샤크", "핑폭각?", "Ready", "Bone Collecter", "GRF SWORD", "서든", "Preber", "프레버", "명인만두 서울대점", "좁밥수비수", "쥐알티", "겐류사이 육두봉", "반다이크", "농협신", "쁘이훈",
		"노래하는메시", "페페", "마샬", "경상도에서태어난아기를영국에서길렀더니내가나왔다", "soy el mas pro", "Ricardo", "drogba", "드록바", "HYNN", "루니", "제주스", "네테로"
	];
...
constructor(){
	...
	this.SecurityPatchLevel		= "2021.06.01c";	// UMUX 보안 패치 수준(건드리지 마시오)
